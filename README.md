...
| ------------------ | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Windows            | `khulla-<version>-windows-x64-setup.exe` | **Recommended.** Run the installer - Start Menu entry + uninstaller, no admin required. SmartScreen warns about an unknown publisher because the build is unsigned - _More info_ → _Run anyway_. |
...
**Prerequisites** - [FVM](https://fvm.app) to pin the Flutter SDK, plus platform toolchains: Visual Studio with the _Desktop development with C++_ workload for Windows; `clang`, `cmake`, `ninja-build`, `libgtk-3-dev` for Linux; Xcode for macOS/iOS; Java 17 plus the Android SDK for the APK.
